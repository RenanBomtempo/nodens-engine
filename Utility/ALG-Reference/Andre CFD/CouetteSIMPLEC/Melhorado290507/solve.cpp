    ...
    //{ Nao usa para Couette
        do // Ciclo B -------------------------------------------------------------
        {
            ...
            do // Ciclo C ---------------------------------------------------------
            {
                ...
                corrige_Velocidades_Densidades_Pressoes( malha, 0.000001 );
...
int condicaoInicial( Grid *malha )        
{
    ...
    if(malha->firstGridCell != NULL)
    {
        ...
        // Variaveis de fluido
            ...
            celula->uAtual = 0.01*VEL_PAREDE; // repouso
...
int calculaVelocidadesNasFaces( Grid *malha )
{
    ...
    if(malha->firstGridCell != NULL)
    {
        ...
        for(celula = malha->firstGridCell;
            ...
            celula = celula->next)
        {
            if( celula->active && celula->type == 'b' )
            {
...
int atualizaParametrosDaIteracaoAnterior( Grid *malha, char ciclo )
{
    ...
    &&  ciclo != 'C' )
    {
        cout << "Na funcao atualizaParametrosDaIteracaoAnterior():\n"; 
        IMPRIMIR_ID_CICLO_INEXISTENTE();
        return INSUCESSO;
    }
...
int convergenciaVelocidades( Grid *malha )
{
    ...
    {
        cout << "Na funcao convergenciaVelocidades():\n" << endl;
        IMPRIMIR_ERRO_MALHA_VAZIA();
        return ERRO_MALHA_VAZIA;
    }
