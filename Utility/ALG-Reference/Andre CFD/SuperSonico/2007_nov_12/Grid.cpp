    ...
    M_oeste(1.0)
{
    ...
    menorResiduo = MENOR_RESIDUO_INICIAL;
}
...
/*______________________________________________________________________________
    ...
    Imprime os resultados na saida.
______________________________________________________________________________*/
int Grid::print( long NumIter )
{
    Cell *gridCell;

    ofstream outputFile( "outputESSPP.txt", ios::trunc );

    // Titulo
    outputFile << "# ------------------------------ " << endl
               << "# Resultados # no tempo t=" << this->tempoAtual << endl
               << "# ------------------------------ " << endl
               << "# Nivel de refinamento: " << this->firstGridCell->level << endl  // so no caso uniforme!
               << "# Iteracoes do tempo: " << NumIter << endl
    ...
    // obs: y_norm = (y*Re_x)/x
    gridCell = firstGridCell;
    for(gridCell = firstGridCell->south->west; 
        gridCell->direcaoFronteira != FRONTEIRA_SUL; 
        gridCell = gridCell->south) 
    {
            outputFile.precision(9);
            outputFile << setw(12) << (gridCell->centerY * sqrt(REYNOLDS_SUPERSONICO)) / gridCell->centerX
                       << "," << setw(12) <<  gridCell->TAtual /TEMPERATURA_LIVRE
                       << "," << setw(12) <<  gridCell->pAtual /PRESSAO_LIVRE
                       << "," << setw(12) <<  gridCell->uAtual / VEL_LIVRE
                       << "," << setw(12) <<  (gridCell->uAtual + gridCell->vAtual) / VEL_SOM << endl;
    }
...
void Grid::cortarMalha( double nivel = 0.5 )
{      
     Cell *celula, *celula2;
     
     if(nivel < 0.0 || nivel > 1.0)
              nivel = 0.5;
              
     if(firstGridCell != NULL)
     {
        for(celula = firstGridCell; 
            celula != 0;
            celula = celula->next)
        {
            // Inativa todas as celulas acima acima de um certo nivel N (centerY)
            if(celula->centerY > (nivel * sideLength))
               celula->active = false;
              
            // Para as celulas logo abaixo do nivel N, ajusta novo north para as celulas
            else if(celula->centerY >= 
                   (nivel * sideLength - celula->faceLength))
            {
                   for(celula2 = firstGridCell;
                       celula2 != NULL &&
                       celula2->centerX != celula->centerX;
                       celula2 = celula2->west)
                   {}
                   celula->north = celula2;
                   celula2->centerY = celula->centerY + celula->faceLength;
                   celula2->south = celula;
                   
                   if(celula2->centerX != celula2->halfFaceLength || 
                      celula2->centerX != sideLength - celula2->halfFaceLength)
                        celula2->active = true;
            } 
