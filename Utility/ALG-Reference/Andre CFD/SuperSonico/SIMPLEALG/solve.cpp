...
/*______________________________________________________________________________
    ...
    o escoamento supersonico na placa plana.
    ...
    { 
        ...
        do // Ciclo B -------------------------------------------------------------
        {
            ...
            do // Ciclo C ---------------------------------------------------------
            {
                ...
                // Verifica convergencia da pressao
            ...
            // Verifica convergencia da velocidade
...
int condicaoInicial( Grid *malha )        
{
    ...
    if(malha->firstGridCell != NULL)
    {
		...
		// Variaveis de fluido
			...
			celula->uAtual = VEL_LIVRE; // repouso
			...
			celula->EtAtual = celula->rhoAtual
                           * (
                                ( R*(celula->TAtual) ) / (GAMMA-1.0) // e (minusculo!)
                                + (
...
int calculaCoeficientesEnergia( Grid *malha )
{
    ...
    if(malha->firstGridCell != NULL)
    {
            ...
            celula = celula->next)
        {
            ...
            // Calculo do termo fonte da equacao da energia
            B = celula->EtAnterior - ( malha->deltaT / malha->sideLength ) 
                                     * (
            ...
            // Inicia o vetor b da equacao Ax = b da energia
            celula->b_Energia = B;

            // Insere coeficientes dos vizinhos na mesma lista encadeada
            if( celula->east->type == 'b' && celula->east->active == true )
                celula->insereElement( F_e, celula->east, &(celula->firstElementEnergia) );
            else    // Vai pro lado direito da equacao Ax = b pois leste e' fronteira
                celula->b_Energia -= ( F_e * condicaoFronteira( celula, ID_VAR_Et, 'e', false ) );

            if( celula->west->type == 'b' )
                celula->insereElement( F_w, celula->west, &(celula->firstElementEnergia) );
            else    // Vai pro lado direito da equacao Ax = b pois oeste e' fronteira
                celula->b_Energia -= ( F_w * condicaoFronteira( celula, ID_VAR_Et, 'w', false ) );

            if( celula->north->type == 'b' )
                celula->insereElement( F_n, celula->north, &(celula->firstElementEnergia) );
            else    // Vai pro lado direito da equacao Ax = b pois norte e' fronteira
                celula->b_Energia -= ( F_n * condicaoFronteira( celula, ID_VAR_Et, 'n', false ) );

            if( celula->south->type == 'b' )
                celula->insereElement( F_s, celula->south, &(celula->firstElementEnergia) );
            else    // Vai pro lado direito da equacao Ax = b pois sul e' fronteira
                celula->b_Energia -= ( F_s * condicaoFronteira( celula, ID_VAR_Et, 's', false ) );
        }
    ...
    {
        IMPRIMIR_ERRO_MALHA_VAZIA();
        cout << "Erro acima ocorreu em calculaCoeficientesEnergia()" << endl;
        return ERRO_MALHA_VAZIA;
    }
...
______________________________________________________________________________*/
int atualizaOutrosEscalares( Grid *malha )
{
    Cell *celula;
    ...
    if(malha->firstGridCell != NULL)
    {
	    ...
	    EnergiaInterna = ( celula->EtAtual / celula->rhoAtual ) - 0.5 *
		( AO_QUADRADO(celula->uAtual) + AO_QUADRADO(celula->vAtual) );

	    // calcula temperatura
	    celula->TAtual = EnergiaInterna / ( R / (GAMMA-1.0) );
    ...
    {
        IMPRIMIR_ERRO_MALHA_VAZIA();
        cout << "Erro acima ocorreu em atualizaTemperaturasDensidades()" << endl;
        return ERRO_MALHA_VAZIA;
    }
    ...
    if( varLeste != NULL )
    {
        ...
        else    // Celula imagina'ria "fronteira" (condicao de fronteira)
        {
            switch( id_variavel )
            {
                ...
                case ID_VAR_U:
                    *varLeste = condicaoFronteira( celula, ID_VAR_U, 'e', false );
                break;

                case ID_VAR_V:
                    *varLeste = condicaoFronteira( celula, ID_VAR_V, 'e', false );
                break;
    ...
    if( varOeste != NULL )
    {
        ...
        else // Celula imagina'ria "fronteira" (condicao de fronteira)
        {
            switch( id_variavel )
            {
                ...
                case ID_VAR_U:
                    *varOeste = condicaoFronteira( celula, ID_VAR_U, 'w', false );
                break;

                case ID_VAR_V:
                    *varOeste = condicaoFronteira( celula, ID_VAR_V, 'w', false );
                break;
    ...
    if( varNorte != NULL )
    {
        ...
        else        // Celula imagina'ria "fronteira" (condicao de fronteira)
        {
            switch( id_variavel )
            {
                ...
                case ID_VAR_U:
                    *varNorte = condicaoFronteira( celula, ID_VAR_U, 'n', false );
                break;

                case ID_VAR_V:
                    *varNorte = condicaoFronteira( celula, ID_VAR_V, 'n', false );
                break;
    ...
    if( varSul != NULL )
    {
        ...
        else // Celula imagina'ria "fronteira" (condicao de fronteira)
        {
            switch( id_variavel )
            {
                ...
                case ID_VAR_U:
                    *varSul = condicaoFronteira( celula, ID_VAR_U, 's', false );
                break;

                case ID_VAR_V:
                    *varSul = condicaoFronteira( celula, ID_VAR_V, 's', false );
                break;
