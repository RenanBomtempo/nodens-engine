...
/*______________________________________________________________________________
                    ...
                    Funcao verificaVizFicticio()

    Verifica se o vizinho em alguma direcao de uma celula e' volume ficticio,
considerando niveis de refinamento possivelmente diferentes e retorna true se,
e somente se, ele e' ficticio.

    Parametros:
        celula: apontador para a direcao desejada da celula
        levelPrincipal: nivel de refinamento da celula principal.

    Criado por Henrique dia 06/12/2007.
______________________________________________________________________________*/

bool verificaVizFicticio( Cell *celula, int &levelPrincipal )
{
    double Var = 0.0,
           S_e = 0.0, S_w = 0.0,
           uVizLeste = 0.0, uVizOeste = 0.0;

    // Se o vizinho e' maior ou igual a celula principal
    if( levelPrincipal == celula->level )
    {
        while( celula->type == 'w' )
            celula = celula->singleConnector;

        return celula->volumeFicticio;
    }

    // Senao, os vizinhos sao menores que a celula pricipal
    while( celula->type == 'w' )
        celula = celula->doubleConnector1;
