...
void desenharMalha( Grid *malha )
{
   Cell *celula;
    ...
    {  
        ...
        if( celula->active && !celula->volumeFicticio )
        {
            // ---------------- Desenha malha ---------------------
...
// Funcao cor de grafico ========================================
// 
// Equacao parametrica de uma curva no espaco RGB.
//
void funcaoCor(double t)
{
    // Tratamento diferente para dois intervalos
    
    // Intervalo Azul - Aqua: (0 <= t < 0.25)
    if(t >= 0.0 && t < 0.35)
    {
        glColor3f(0.0, (20.0/7.0) * t, 1.0);
    } 

    // Intervalo Aqua - Verde (0.25 <= t < 0.5)
    else if(t >= 0.35 && t < 0.5)
    {
        glColor3f(0.0, 1.0, -(20.0/3.0) * t + (10.0/3.0));
    }

    // Intervalo Verde - Amarelo (0.5 <= t < 0.75)
    else if(t >= 0.5 && t < 0.65)
    {
        glColor3f((20.0/3.0) * t - (10.0/3.0), 1.0, 0.0);
    }

    // Intervalo Amarelo - Vermelho (0.75 <= t <= 1.0)
    else if(t >= 0.65 && t <= 1.0)
    {
        glColor3f(1.0, -(20.0/7.0) * t + (20.0/7.0), 0.0);
    }

    // Valores abaixo
    else if(t < 0.0)
    {
        glColor3f COR_LIM_INF;
    }
    
    // Valores acima
    else if(t > 1.0)
    {
        glColor3f COR_LIM_SUP;
    }
}
...
void desenharDensidade( Grid *malha )
{
        ...
        celula = celula->next)
    {
        ...
        {
            const double b = 1.0 - LEGENDA_LIM_SUP_DENSIDADE 
                        / (LEGENDA_LIM_SUP_DENSIDADE - LEGENDA_LIM_INF_DENSIDADE);
            const double a = 1.0 / (LEGENDA_LIM_SUP_DENSIDADE - LEGENDA_LIM_INF_DENSIDADE);

            funcaoCor(a * celula->rhoAtual + b);
...
void desenharTemperatura( Grid *malha )
{
        ...
        celula = celula->next)
    {
        if(celula->volumeFicticio == false && celula->active)
        {
            const double b = 1.0 - (LEGENDA_LIM_SUP_TEMPERATURA /
                         (LEGENDA_LIM_SUP_TEMPERATURA - LEGENDA_LIM_INF_TEMPERATURA ));
            const double a = 1.0 / (LEGENDA_LIM_SUP_TEMPERATURA - LEGENDA_LIM_INF_TEMPERATURA);

            funcaoCor(a * celula->TAtual + b);
...
/* -----------------------------------------------
    Desenha valores da velocidade em cada celula
-------------------------------------------------- */
void desenharMach( Grid *malha )
{
        ...
        celula = celula->next)
    {
        if(celula->volumeFicticio == false && celula->active)
        {
            const double b = 1.0 - (LEGENDA_LIM_SUP_MACH /
                         (LEGENDA_LIM_SUP_MACH - LEGENDA_LIM_INF_MACH ));
            const double a = 1.0 / (LEGENDA_LIM_SUP_MACH - LEGENDA_LIM_INF_MACH);
            
            mach = ( sqrt( AO_QUADRADO(celula->uAtual) + AO_QUADRADO(celula->vAtual)))
                    /VEL_SOM;
            funcaoCor(a * mach + b);
...
void desenharPressao( Grid *malha )
{
        ...
        celula = celula->next)
    {
        if(celula->volumeFicticio == false && celula->active)
        {
            const double b = 1.0 - LEGENDA_LIM_SUP_PRESSAO 
                        / (LEGENDA_LIM_SUP_PRESSAO - LEGENDA_LIM_INF_PRESSAO);
            const double a = 1.0 / (LEGENDA_LIM_SUP_PRESSAO - LEGENDA_LIM_INF_PRESSAO);

            funcaoCor(a * celula->pAtual + b);
...
void desenharValores( Grid *malha , int QualGrafico )
{
        ...
        malhaCell = malhaCell->next)
    {
        if(malhaCell->volumeFicticio == false && malhaCell->active)
        {
            ...
            switch(QualGrafico)
            {
                case ID_GRAFICO_DENS:  //imprimir densidade
                    sprintf(valor, "%1.2f", (malhaCell->rhoAtual) );    
                break;

                case ID_GRAFICO_VELO:  //imprimir velocidade
                    sprintf(valor, "%1.1f", sqrt(    AO_QUADRADO(malhaCell->uAtual) 
                                                + AO_QUADRADO(malhaCell->vAtual)) );    
                break;   

                case ID_GRAFICO_TEMP:  //imprimir temperatura
                    sprintf(valor, "%1.0f", malhaCell->TAtual );    
                break;       

                case ID_GRAFICO_PRES:  //imprimir pressao
                    sprintf(valor, "%1.0f", (malhaCell->pAtual) );
                break;

                case ID_GRAFICO_MACH:  //imprimir mach 
                    sprintf(valor, "%1.0f", sqrt( AO_QUADRADO(malhaCell->uAtual) 
                                                + AO_QUADRADO(malhaCell->vAtual)) / VEL_SOM );
                break;
            ...
            if( QualGrafico != ID_GRAFICO_GRIDPOS )
        ...
        // imprimir numero de ordem da celula na malha
        if( QualGrafico == ID_GRAFICO_GRIDPOS )
        {
            sprintf(valor, "%3d", (malhaCell->gridPosition) );
                    ...
                    int QualGrafico )
{
    ...
    const float DELTAYGRAD_PRES = (YSUPBARRA - YINFBARRA) 
                                    / 
                                 ( (float) PASSOSPRES );
    const float DELTAYGRAD_MACH = (YSUPBARRA - YINFBARRA)
                                        /
                                  ( (float) PASSOSMACH );

    // Desenha titulo exceto para velocidade
    if( QualGrafico != ID_GRAFICO_VELO && QualGrafico != ID_GRAFICO_GRIDPOS )
    {
        // Cor preta para as fontes
        glColor3f(0.0f, 0.0f, 0.0f);
    ...
    // Desenha elementos comuns, exceto para a velocidade e residuos
    if( QualGrafico != ID_GRAFICO_VELO 
    &&  QualGrafico != ID_GRAFICO_RESI 
    &&  QualGrafico != ID_GRAFICO_GRIDPOS )
    {
    ...
    switch( QualGrafico )
    {
      case ID_GRAFICO_DENS: // ----------------- DENSIDADE ---------------------------
        glColor3f(0.0f, 0.0f, 0.0f);
        ...
        for( i = 0; i <= PASSOSDENS; i++ )
        {
            ...
            glBegin(GL_QUADS);
                ...
                // Coeficientes a1 e b1 da funcao linear que mapeia rho em t da funcaoCor
                const double a1 = 1.0 / (LEGENDA_LIM_SUP_DENSIDADE - LEGENDA_LIM_INF_DENSIDADE);
                const double b1 = 1.0 - LEGENDA_LIM_SUP_DENSIDADE 
                        / (LEGENDA_LIM_SUP_DENSIDADE - LEGENDA_LIM_INF_DENSIDADE);

                // Coeficientes a2 e b2 da funcao linear que mapeia i em rho
                const double a2 = (LEGENDA_LIM_SUP_DENSIDADE - LEGENDA_LIM_INF_DENSIDADE) 
                                    / ((double)PASSOSDENS);
                const double b2 = LEGENDA_LIM_INF_DENSIDADE;

                funcaoCor(a1 * (a2 * ((double)i) + b2) + b1);
                ...
                funcaoCor(a1 * (a2 * ((double)(i-1)) + b2) + b1);
            ...
            if( i%2 == 0 )
            {
                ...
                const double a2 = (LEGENDA_LIM_SUP_DENSIDADE - LEGENDA_LIM_INF_DENSIDADE) 
                                    / ((double)PASSOSDENS);
                const double b2 = LEGENDA_LIM_INF_DENSIDADE;
                double valorRho = (a2 * ((double)i) + b2);
      ...
      case ID_GRAFICO_TEMP: // ---------------------- TEMPERATURA -----------------------
        glColor3f(0.0f, 0.0f, 0.0f);
        // Desenhar titulo da legenda          
        ...
        for( i = 0; i <= PASSOSTEMP; i++ )
        {
            ...
            {                         
            glBegin(GL_QUADS);
                // Coeficientes a1 e b1 da funcao linear que mapeia Temp em t da funcaoCor
                const double a1 = 1.0 / (LEGENDA_LIM_SUP_TEMPERATURA - LEGENDA_LIM_INF_TEMPERATURA);
                const double b1 = 1.0 - LEGENDA_LIM_SUP_TEMPERATURA 
                        / (LEGENDA_LIM_SUP_TEMPERATURA - LEGENDA_LIM_INF_TEMPERATURA);

                // Coeficientes a2 e b2 da funcao linear que mapeia i em Temp
                const double a2 = (LEGENDA_LIM_SUP_TEMPERATURA - LEGENDA_LIM_INF_TEMPERATURA) 
                                    / ((double)PASSOSTEMP);
                const double b2 = LEGENDA_LIM_INF_TEMPERATURA;
      ...
      case ID_GRAFICO_PRES:  // ------------------------- PRESSAO -------------------------
        ...
        for( i = 0; i <= PASSOSPRES; i++ )
        {
            ...
            glBegin(GL_QUADS);
                ...
                // Coeficientes a1 e b1 da funcao linear que mapeia Pressao em t da funcaoCor
                const double a1 = 1.0 / (LEGENDA_LIM_SUP_PRESSAO - LEGENDA_LIM_INF_PRESSAO);
                const double b1 = 1.0 - LEGENDA_LIM_SUP_PRESSAO 
                        / (LEGENDA_LIM_SUP_PRESSAO - LEGENDA_LIM_INF_PRESSAO);

                // Coeficientes a2 e b2 da funcao linear que mapeia i em Pressao
                const double a2 = (LEGENDA_LIM_SUP_PRESSAO - LEGENDA_LIM_INF_PRESSAO) 
                                    / ((double)PASSOSPRES);
                const double b2 = LEGENDA_LIM_INF_PRESSAO;
     ...
     case ID_GRAFICO_MACH:  // ------------------------- MACH -------------------------
        ...
        for( i = 0; i <= PASSOSMACH; i++ )
        {
            ...
            glBegin(GL_QUADS);
                ...
                // Coeficientes a1 e b1 da funcao linear que mapeia Pressao em t da funcaoCor
                const double a1 = 1.0 / (LEGENDA_LIM_SUP_MACH - LEGENDA_LIM_INF_MACH);
                const double b1 = 1.0 - LEGENDA_LIM_SUP_MACH 
                        / (LEGENDA_LIM_SUP_MACH - LEGENDA_LIM_INF_MACH);

                // Coeficientes a2 e b2 da funcao linear que mapeia i em Pressao
                const double a2 = (LEGENDA_LIM_SUP_MACH - LEGENDA_LIM_INF_MACH) 
                                    / ((double)PASSOSMACH);
                const double b2 = LEGENDA_LIM_INF_MACH;

                funcaoCor(a1 * (a2 * ((double)i) + b2) + b1);
                                     
                glVertex3d( (XESQBARRA/zoom) - deslocamentoX,
                            ( YINFBARRA + (i*DELTAYGRAD_MACH) )/zoom - deslocamentoY,
                            0.0 );
                ...
                glVertex3d( (XDIRBARRA/zoom) - deslocamentoX,
                            ( YINFBARRA + ((i-1)*DELTAYGRAD_MACH) )/zoom - deslocamentoY,
                            0.0 );
            ...
            ||  i == 12 )
            {
                ...
                // Coeficientes a2 e b2 da funcao linear que mapeia i em Mach 
                const double a2 = (LEGENDA_LIM_SUP_MACH - LEGENDA_LIM_INF_MACH) 
                                    / ((double)PASSOSMACH);
                const double b2 = LEGENDA_LIM_INF_MACH;
                double valorM = (a2 * ((double)i) + b2);
                sprintf( Texto, "%1.0f", valorM );

                drawString( Texto, GLUT_BITMAP_HELVETICA_12,
                          ((XESQBARRA-0.06)/zoom) - deslocamentoX,
                          ( (YINFBARRA-0.01) + (i*DELTAYGRAD_MACH) )/zoom - deslocamentoY, 
                           0.0 );     
            }
        }
      break;

      case ID_GRAFICO_RESI:  // ------------------------- Residuos -------------------------
         ...
         //desenha o retangulo branco da legenda do grafico de res�uos
    ...
    if( QualGrafico != ID_GRAFICO_RESI 
    &&  QualGrafico != ID_GRAFICO_VELO 
    &&  QualGrafico != ID_GRAFICO_GRIDPOS)
    {
...
void desenharResiduo( Grid *malha, int iteracaoAtual )
{
    ...
    for(i = 1; i < MAXITER_GRAFICO_RESIDUOS; i++)
    {
        ...
        &&  malha->residuosP[(j+1)%MAXITER_GRAFICO_RESIDUOS] > LIMITE_INFERIOR_RESIDUO)
        {
            ...
            drawLine( -L / 2.0 + ( ((double)(i-1)) / MAXITER_GRAFICO_RESIDUOS) * L,
                    +L * 0.488 + 0.98*( (log10(malha->residuosP[j])) 
                                                / fabs(log10(malha->menorResiduo)) ) * L,
                    0,
                    -L / 2.0 + ( ((double)i) / MAXITER_GRAFICO_RESIDUOS) * L,
                    +L * 0.488 + 0.98*(log10(malha->residuosP[(j+1)%MAXITER_GRAFICO_RESIDUOS])
                                                / fabs(log10(malha->menorResiduo)) ) * L,
                    0 );
        }
        ...
        &&  malha->residuosE[(j+1)%MAXITER_GRAFICO_RESIDUOS] > LIMITE_INFERIOR_RESIDUO)
        {
            glColor3f(1.0f,0.1f,0.0f); // vermelho
            drawLine( -L / 2.0 + ( ((double)(i-1)) / MAXITER_GRAFICO_RESIDUOS) * L,
                    +L * 0.488 + 0.98*( log10(malha->residuosE[j]) / fabs(log10(malha->menorResiduo)) ) * L,
                    0,
                    -L / 2.0 + ( ((double)i) / MAXITER_GRAFICO_RESIDUOS) * L,
                    +L * 0.488 + 0.98*( log10(malha->residuosE[(j+1)%MAXITER_GRAFICO_RESIDUOS])
                                        / fabs(log10(malha->menorResiduo)) ) * L,
                    0 );
        }
        ...
        &&  malha->residuosU[(j+1)%MAXITER_GRAFICO_RESIDUOS] > LIMITE_INFERIOR_RESIDUO)
        {
            glColor3f(0.0f,0.0f,1.0f); // azul puro
            drawLine( -L / 2.0 + ( ((double)(i-1)) / MAXITER_GRAFICO_RESIDUOS) * L,
                    +L * 0.488 + 0.98*( log10(malha->residuosU[j]) / fabs(log10(malha->menorResiduo)) ) * L,
                    0,
                    -L / 2.0 + ( ((double)i) / MAXITER_GRAFICO_RESIDUOS) * L,
                    +L * 0.488 + 0.98*( log10(malha->residuosU[(j+1)%MAXITER_GRAFICO_RESIDUOS])
                                        / fabs(log10(malha->menorResiduo)) ) * L,
                    0 );
        }
        ...
        &&  malha->residuosV[(j+1)%MAXITER_GRAFICO_RESIDUOS] > LIMITE_INFERIOR_RESIDUO)
        {
            glColor3f(0.0, 0.5, 0.5); // azul esverdeado
            drawLine( -L / 2.0 + ( ((double)(i-1)) / MAXITER_GRAFICO_RESIDUOS) * L,
                    +L * 0.488 + 0.98*( log10(malha->residuosV[j]) / fabs(log10(malha->menorResiduo)) ) * L,
                    0,
                    -L / 2.0 + ( ((double)i) / MAXITER_GRAFICO_RESIDUOS) * L,
                    +L * 0.488 + 0.98*( log10(malha->residuosV[(j+1)%MAXITER_GRAFICO_RESIDUOS])
                                        / fabs(log10(malha->menorResiduo)) ) * L,
                    0 );
        }
        ...
        &&  malha->residuosM[(j+1)%MAXITER_GRAFICO_RESIDUOS] > LIMITE_INFERIOR_RESIDUO)
        {
            glColor3f(0.7f,0.7f,0.0f); // amarelo escuro
            drawLine( -L / 2.0 + ( ((double)(i-1)) / MAXITER_GRAFICO_RESIDUOS) * L,
                    +L * 0.488 + 0.98*( log10(malha->residuosM[j]) / fabs(log10(malha->menorResiduo)) ) * L,
                    0,
                    -L / 2.0 + ( ((double)i) / MAXITER_GRAFICO_RESIDUOS) * L,
                    +L * 0.488 + 0.98*( log10(malha->residuosM[(j+1)%MAXITER_GRAFICO_RESIDUOS])
                                        / fabs(log10(malha->menorResiduo)) ) * L,
                    0 );
        }
    }
    ...
    // Marcador horizontais 
    glLineWidth(1.0);
    int k;
    const int MARCADORES = 5;
    for(k = 0; k <= MARCADORES; k++){
        sprintf(valor, "1E%2.2f", (
                                    ( ((double)k) 
                                        * log10(malha->menorResiduo)
                                    )
                                    / ((double)MARCADORES)
                                ) );
        ...
        drawLine(-L * 0.5, +L * 0.488 - 0.98*( (
                                                ( ((double)k) 
                                                    * fabs(log10(malha->menorResiduo))
                                                )
                                                / ((double)MARCADORES)
                                              ) / fabs(log10(malha->menorResiduo)) ) * L, 0,
                 +L * 0.5, +L * 0.488 - 0.98*( (
                                                ( ((double)k) 
                                                    * fabs(log10(malha->menorResiduo))
                                                )
                                                / ((double)MARCADORES)
                                              ) / fabs(log10(malha->menorResiduo)) ) * L, 0); // linha
    }

    // Marcador de cota do grad. conjugado =======================================================
    sprintf(valor, "GBCE" );

    glColor3f(0.0,0.0,0.0);
    // risco pequeno no eixo y
    drawLine(-L * 0.52, 
             +L * 0.488 + 0.98* ( log10(COTA_GRADCONJUGADO) / fabs(log10(malha->menorResiduo)) ) * L, 
              0,
             -L * 0.5, 
             +L * 0.488 + 0.98* ( log10(COTA_GRADCONJUGADO) / fabs(log10(malha->menorResiduo)) ) * L, 
              0 );
    ...
    drawString(valor, GLUT_BITMAP_HELVETICA_12,
            -L * 0.588, 
            +L * 0.48 + 0.98* ( log10(COTA_GRADCONJUGADO) / fabs(log10(malha->menorResiduo)) ) * L, 
            0);
    ...
    // Marcador de cota do grad. conjugado =======================================================
    sprintf(valor, "GBCE P'" );

    glColor3f(0.0,0.0,0.0);
    // risco pequeno no eixo y
    drawLine(-L * 0.52, 
             +L * 0.488 + 0.98* ( log10(COTA_GRADCONJUGADO_PRESSAO) / fabs(log10(malha->menorResiduo)) ) * L, 
              0,
             -L * 0.5, 
             +L * 0.488 + 0.98* ( log10(COTA_GRADCONJUGADO_PRESSAO) / fabs(log10(malha->menorResiduo)) ) * L, 
              0 );
    ...
    drawString(valor, GLUT_BITMAP_HELVETICA_12,
            -L * 0.6, 
            +L * 0.48 + 0.98* ( log10(COTA_GRADCONJUGADO_PRESSAO) / fabs(log10(malha->menorResiduo)) ) * L, 
            0);
    ...
    // Marcador de cota do grad. conjugado =======================================================
    sprintf(valor, "GBCE T" );

    glColor3f(0.0,0.0,0.0);
    // risco pequeno no eixo y
    drawLine(-L * 0.52, 
             +L * 0.488 + 0.98* ( log10(COTA_GRADCONJUGADO_TEMPERATURA) / fabs(log10(malha->menorResiduo)) ) * L, 
              0,
             -L * 0.5, 
             +L * 0.488 + 0.98* ( log10(COTA_GRADCONJUGADO_TEMPERATURA) / fabs(log10(malha->menorResiduo)) ) * L, 
              0 );
    ...
    drawString(valor, GLUT_BITMAP_HELVETICA_12,
            -L * 0.6, 
            +L * 0.48 + 0.98* ( log10(COTA_GRADCONJUGADO_TEMPERATURA) / fabs(log10(malha->menorResiduo)) ) * L, 
            0);
