...
------------------------------------------------------------------------------*/
void desenharVelocidade( Grid *grid )
{
    ...
    while( gridCell != 0 )  // Desenha um vetor em cada celula
    { 
        tamanhoHorizontal = (gridCell->uAtual/(4.0*(VEL_SOM)))
                            *1.5*(gridCell->halfFaceLength);
        tamanhoVertical = (gridCell->vAtual/(2.5*(VEL_SOM)))
                            *1.5*(gridCell->halfFaceLength);

        // Inicio do segmento do vetor
        xA = gridCell->centerX - 0.5*tamanhoHorizontal;
        yA = gridCell->centerY - 0.5*tamanhoVertical;

        // Final do segmento do vetor
        xB = gridCell->centerX + 0.5*tamanhoHorizontal; 
        yB = gridCell->centerY + 0.5*tamanhoVertical;
        ...
        // Continuar o loop para a proxima celula
        gridCell = gridCell->next;
    }
}
        ...
        gridCell = gridCell->next)
    {
        ...
        switch(QualGrafico){
            ...
            case 1:  //imprimir densidade
                sprintf(valor, "%1.3f", gridCell->rhoAtual );    
            break;

            case 2:  //imprimir temperatura
                sprintf(valor, "%1.0f", gridCell->TIterAnterior );    
            break;

            case 3:  //imprimir velocidade
                sprintf(valor, "%1.0f", (gridCell->uAtual+gridCell->vAtual) );    
            break;          
...
{
    char string_tempo[11];

    // Preencher string com valores
    sprintf(string_tempo, "%1.11f", grid->tempoAtual);  //imprimir tempo
