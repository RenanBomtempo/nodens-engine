   ...
   Cell *celula;
    ...
    {  
        ...
        if( celula->active )
        {
            ...
            drawLine(-LHORI/2+(celula->centerX - celula->halfFaceLength),
                    -LHORI/2+(celula->centerY + celula->halfFaceLength), 
                    0,
                        -LHORI/2+(celula->centerX + celula->halfFaceLength),
                        -LHORI/2+(celula->centerY + celula->halfFaceLength),
                        0);
...
void desenharValores( Grid *grid , int QualGrafico )
{
    ...
    {
        if(gridCell->volumeFicticio == false && gridCell->active)
        {
            ...
            switch(QualGrafico)
            {
                ...
                case 3:  //imprimir velocidade
                    sprintf(valor, "%1.1f", pow(    AO_QUADRADO(gridCell->uAtual) 
                                                + AO_QUADRADO(gridCell->vAtual), 0.5) );    
                break;          
...
-------------------------------------------------- */
void desenharPlacasPlanas( Grid *grid )
{    
    // Desenhar placas planas na forma de poligono
    glBegin(GL_POLYGON);
        glColor3f(0.5f,0.5f,0.5f);       
        glVertex3d(-LHORI/2.0 + LHORI/AO_QUADRADO((double)grid->refinamentoMaximo),
                   -LHORI/2.0 + LHORI/AO_QUADRADO((double)grid->refinamentoMaximo),
                   0);

        glColor3f(0.48f,0.48f,0.48f);
        glVertex3d(-(LHORI/2.0) + LHORI/AO_QUADRADO((double)grid->refinamentoMaximo),
                   -1.1*(LHORI/2) + LHORI/AO_QUADRADO((double)grid->refinamentoMaximo),
                   0);

        glColor3f(0.55f,0.55f,0.55f);
        glVertex3d((LHORI/2) - LHORI/AO_QUADRADO((double)grid->refinamentoMaximo),
                   -1.1*(LHORI/2) + LHORI/AO_QUADRADO((double)grid->refinamentoMaximo),
                   0);

        glColor3f(0.38f,0.38f,0.38f);
        glVertex3d((LHORI/2) - LHORI/AO_QUADRADO((double)grid->refinamentoMaximo),
                   -(LHORI/2) + LHORI/AO_QUADRADO((double)grid->refinamentoMaximo),
                   0);
    glEnd();
