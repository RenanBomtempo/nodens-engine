    ...
    do // Ciclo A -------------------------------------------------------------
    {
        ...
        do // Ciclo B -------------------------------------------------------------
        {
            ...
            do // Ciclo C ---------------------------------------------------------
            {
                ...
                corrige_Velocidades_Densidades_Pressoes( malha, 0.01 );
    ...
    if(malha->firstGridCell != NULL)
    {
            ...
            celula = celula->next)
        {
            ...
            if( celula->active )
            {
                ...
                if( celula->west->type == 'w' && celula->volumeFicticio == true ){
                ...
                else if( celula->east->type == 'w' && celula->volumeFicticio == true ){
                ...
                else if( celula->north->type == 'w' && celula->volumeFicticio == true ){
                ...
                else if( celula->south->type == 'w' && celula->volumeFicticio == true ){
...
int calculaCoeficientesCorrecaoPressao( Grid *malha )
{
    ...
    if(malha->firstGridCell != NULL)
    {
            ...
            celula = celula->next)
        {
            ...
            {
                // Obtem o identificador que mostra qual o tratamento para a fronteira
                tipoFronteira = tipoCondicaoFronteira( celula, ID_VAR_P );
                ...
                // Volume ficticio (fronteira leste)
                    ...
                    if(tipoFronteira == CONDICAO_FRONTEIRA_PRESCRITA)
                    {
                        ...
                        // B de acordo com 7.84 do Maliska
                        celula->b_MomentoY = 2.0 * valorCondicaoFronteira( celula, ID_VAR_P, 'e' );
                    }
...
______________________________________________________________________________*/
int corrige_Velocidades_Densidades_Pressoes( Grid *malha, double relaxamento = 1.0 )
{
    // Celula da malha
    Cell *celula;
    ...
    if(malha->firstGridCell != NULL)
    {
            ...
            celula = celula->next)
        {
            ...
            if( celula->active && celula->volumeFicticio == false )
            {
                ...
                if( celula->east->type == 'b' && celula->east->active == true)
                    celula->eastFaceU -= celula->d_e * relaxamento 
                ...
                if( celula->west->type == 'b' && celula->west->active == true)
                    celula->westFaceU -= celula->d_w * relaxamento
                ...
                if( celula->north->type == 'b' && celula->north->active == true)
                    celula->northFaceV -= celula->d_n * relaxamento
                ...
                if( celula->south->type == 'b' && celula->south->active == true)
                    celula->southFaceV -= celula->d_s * relaxamento
                ...
                celula->rhoAtual += (relaxamento * celula->pLinha) / ( R * celula->TAtual );

                // Finalmente, corrige a pressao no centro da celula atual
                celula->pAtual = celula->pAtual + relaxamento*celula->pLinha;
            }
        }
            ...
            celula = celula->south)
        {
            if( celula->east->type == 'w' && celula->volumeFicticio )
            {
                cout << "u no leste ficticio = " << celula->uAtual << endl;

                // u no leste
                celula->uAtual += relaxamento * (malha->entradaMassa/malha->saidaMassa) 
                                * (celula->west->uAtual - celula->west->uIterAnterior);

                // v no leste
                celula->vAtual += relaxamento * (celula->west->vAtual - celula->west->vIterAnterior);
        ...
        for(celula = celula->west;
            ...
            celula = celula->west)
        {
            if( celula->south->type == 'w' && celula->volumeFicticio )
            {
                // u no sul
                celula->uAtual -= relaxamento * (celula->north->uAtual - celula->north->uIterAnterior);

                // v no sul
                celula->vAtual += relaxamento * (celula->north->vAtual - celula->north->vIterAnterior);
        ...
        // para calcular a correcao de variaveis
        for(celula = malha->firstGridCell->west;
            celula->active;
            celula = celula->west)
        {
            if( celula->south->type == 'w' && celula->volumeFicticio )
            {
                // u no norte
                celula->uAtual -= relaxamento * (celula->south->uAtual - celula->south->uIterAnterior);

                // v no norte
                celula->vAtual += relaxamento * (celula->south->vAtual - celula->south->vIterAnterior);
            ...
            if( celula->active )
            {
                ...
                else if( celula->east->type == 'w' && celula->volumeFicticio == true )
                {
                    ...
                    if(tipoFronteira == CONDICAO_FRONTEIRA_SAIDA_MASSA)
                    {
                        // Saida de massa, coeficientes importantes
                        F_P = 1.0;
                        F_w = -1.0;// * (malha->entradaMassa / malha->saidaMassa);
